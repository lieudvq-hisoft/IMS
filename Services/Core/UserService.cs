...
public class UserService : IUserService
{
    ...
    private async Task<Token> GetAccessToken(User user, List<string> roles)
    {
        ...
        return new Token
        {
            ...
            Token_type = "Bearer",
            Expires_in = int.Parse(_config["Jwt:ExpireTimes"]) * 60 * 24,
    ...
    public async Task<ResultModel> GetAccountInfo(string email)
    {
        ...
        try
        {
            var user = _dbContext.User.Include(x => x.UserRoles).ThenInclude(x => x.Role).FirstOrDefault(x => x.Email == email && x.EmailConfirmed);
